...
- For Ubuntu: `takserver_X.X-RELEASEX_all.deb`
- For Rocky/RHEL: `takserver-X.X-RELEASEX.noarch.rpm`

**OPTIONAL:** Also upload signature verification files (same directory):
- Ubuntu: `takserver-public-gpg.key` AND `deb_policy.pol`
- Rocky/RHEL: `takserver-public-gpg.key`

All files must be in the same directory as the scripts!
...
```bash
# Run from the same directory (ubuntu-22.04/ or rocky-9/)
sudo ./caddy-setup.sh
```
